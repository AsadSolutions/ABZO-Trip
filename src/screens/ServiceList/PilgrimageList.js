...
export default function PilgrimageList(props) {
  const {colors} = useTheme();
  const {t} = useTranslation();
  const navigation = useNavigation();
  const isFocused = useIsFocused();
  const storeFilter = useSelector(state => state.booking.pilgrimage_filter);
  ...
  useEffect(() => {
    if (storeFilter) {
      setSubTitle(
        storeFilter.city ? storeFilter.city + ' #' + storeFilter.cat_id : '',
      );
      var post_data = new FormData();
      post_data.append('category_id', storeFilter.cat_id);
      post_data.append('city_id', storeFilter.city_id);
      post_data.append('from_city', storeFilter.city);
      post_data.append('address', storeFilter.address);
      post_data.append('from_country', storeFilter.country);
      post_data.append('package', 'pilgrim');
      // console.log("pilgrimage search", post_data)
   ...
   */
  const onFilter = () => {
    navigation.navigate('PilgrimageFilter');
  };
      ...
      <View style={{flex: 1}}>
        <Animated.FlatList
          ...
          renderItem={({item, index}) => {
            let picture = item.picture_visiting
              ? item.picture_visiting.split(',')[0]
              : 'upload/default.png';
            ...
            return (
              <HotelItem
                ...
                onPress={() => {
                  ...
                  navigation.navigate('PilgrimDetail', {id: item.id});
                }}
              />
            );
          }}
  ...
  return (
    <SafeAreaView style={BaseStyle.safeAreaView} forceInset={{top: 'always'}}>
      ...
      <View style={{padding: 20}}>
        <Button onPress={() => navigation.navigate('CustomPackage')}>
          Customized Package
        </Button>
      </View>
      {renderContent()}
