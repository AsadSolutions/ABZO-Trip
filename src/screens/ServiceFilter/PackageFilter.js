...
import {View, FlatList, TouchableOpacity, ScrollView, Picker} from 'react-native';
import {BaseStyle, BaseColor, useTheme, BaseSetting} from '@config';
import {Header, SafeAreaView, Icon, Text, Tag,Button} from '@components';
...
export default function PackageFilter({navigation}) {
 ...
 //console.warn("Store filter ", storeFilter);
  ...
  const getList = () => {
    const request = new Request(BaseSetting.REACT_APP_BASE_PATH + "ServiceSubCategory?ServiceCategoryId=4&status=true", { method: "GET" });
  ...
  const getCityList = () => {
    const request = new Request(BaseSetting.REACT_APP_BASE_PATH + "City?package=1&allow_booking=1", { method: "GET" });
    fetch(request).then(res => {
      res.json().then(obj => {
        var listItems = [];
        Object.keys(obj).forEach(function (key) {
  ...
  const apply = () => {
    ...
    dispatch(BookingActions.onPACKAGE_FILTER(package_filter));
