...
export default function PilgrimageFilter({navigation}) {
  ...
  const getList = () => {
    const request = new Request(
      BaseSetting.REACT_APP_BASE_PATH +
        'ServiceSubCategory?ServiceCategoryId=5&status=true',
      {method: 'GET'},
    );
    fetch(request)
      .then(res => {
  ...
  const getCityList = () => {
    const request = new Request(
      BaseSetting.REACT_APP_BASE_PATH + 'City?stay=1&allow_booking=1',
      {method: 'GET'},
    );
    fetch(request)
      .then(res => {
  ...
  const apply = () => {
    ...
    dispatch(BookingActions.onPILGRIMAGE_FILTER(pilgrimage_filter));
    navigation.navigate('PilgrimageList');
  };

  return (
    <SafeAreaView style={BaseStyle.safeAreaView} forceInset={{top: 'always'}}>
      <Header
        title="Pilgrimage Filter"
        renderLeft={() => {
